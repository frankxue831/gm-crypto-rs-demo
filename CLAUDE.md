...
- `examples/` — 13 self-verifying cookbook examples; CI builds and runs them all.
  ...
  `sm2_key_exchange` (`sm2-key-exchange`); `tlcp_key_schedule`, `tlcp_record` (`tlcp`); `sm4_xts` (`sm4-xts`).
...
- **Gated examples** need their feature flag (`sm4-aead` for `sm4_aead`/`sm4_ccm`/`sm4_streaming`,
  `sm2-key-exchange` for `sm2_key_exchange`, `tlcp` for `tlcp_key_schedule`/`tlcp_record`, `sm4-xts` for
...
- **CI** runs `cargo fmt --check`, clippy with `--all-features`, `cargo test` (default),
  `cargo test --all-features`, all 13 examples (gated ones each under their minimal
